...
Pedido vigente: **fechar um mini-MVP interno de 5 itens**, sem ampliar o escopo.
...
- MVP-INTERFACE — seleção consistente de subelementos e corpos, seleção por
  área, convenções de teclado, mover/girar, edição nos comandos, descoberta de
  comandos e ergonomia básica.
- MVP-SKETCH — entidades persistentes, solver integrado, cotas gerais, graus de
  liberdade/conflitos, arraste com restrições e snapping/inferências.
- MVP-HISTÓRICO — referências topológicas, grafo de dependências, diagnóstico e
  reparo, parâmetros/expressões, reordenação/supressão e reedição sem duplicação.

Status: **2/5 aceites verificados em desenvolvimento**; MVP-INTERFACE,
MVP-SKETCH e MVP-HISTÓRICO permanecem em execução, sem ampliar o escopo.
...
| SK-04 | P0 · Parcial | Graus de liberdade e conflitos | Identificar sub-restrito, totalmente restrito e sobre-restrito; realçar relações conflitantes e permitir reparo. Revisão com realce e redundâncias disponível; inclusão incompatível via comandos H/V/fixar/cotas X/Y mostra participantes e acesso à revisão. Demais caminhos de edição e diagnóstico de contornos ainda pendentes. |
