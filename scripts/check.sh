...
case "$mode" in
    snapping)
        cmake --build build --target snap_tests -j "${MECACAD_BUILD_JOBS:-6}"
        cd build
        exec ./snap_tests "$@"
        ;;
    corpus)
    ...
    release)
        ...
        ;;
    *)
        echo 'Uso: sh scripts/check.sh [core [teste... ] | corpus [teste... ] | parameters [teste... ] | sketch [teste... ] | snapping [teste... ] | recovery [teste... ] | ui teste... | release]' >&2
        exit 2
        ;;
esac
